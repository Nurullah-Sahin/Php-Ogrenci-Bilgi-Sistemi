...
if(isset($_POST['admingir']) ){

  $sorgu = $db->prepare("SELECT * FROM t_akademisyen where Ad=:ad and Tc=:tc");
  $sorgu->execute( array( 
  'ad' =>$_POST['Ad'],
  'tc' =>$_POST['Tc']));
  $sonuclar=$sorgu->rowCount();

  if ($sonuclar>0) {
  	$_SESSION['kul_ad']=$_POST['Ad'];
  	header("Location:../production/memur.php?$_SESSION");

  }else{

  	header("Location:../production/login.php?durum=basarisiz");
